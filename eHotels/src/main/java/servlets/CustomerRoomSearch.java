...
@WebServlet(name="customerRoomSearch",urlPatterns={"/customerRoomSearch"})
public class CustomerRoomSearch extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String province_state = request.getParameter("province/state");
        String country = request.getParameter("country");
        String hotel_chain = request.getParameter("chain");
        ...
        // sql query
        String sql = "SELECT room.room_id, room.hotel_id, room.price, room.capacity, room.extendable, hotel.hotel_name, hotel.rating, hotel.number_of_rooms, hotel.chain_name, hotel.city, hotel.\"province/state\", hotel.country\n" +
                ...
                "WHERE (booking.start_date IS NULL OR booking.end_date IS NULL OR (? < booking.start_date AND ? < booking.start_date) OR (? > booking.end_date AND ? > booking.end_date))\n" +
                "AND (renting.start_date IS NULL OR renting.end_date IS NULL OR (? < renting.start_date AND ? < renting.start_date) OR (? > renting.end_date AND ? > renting.end_date))";
        ...
        try (Connection con = db.getConnection()) {
            ...
            if (price != null && !price.isEmpty()) {
                stmt.setInt(index++, Integer.parseInt(price));
            }
            ...
            request.getRequestDispatcher("roomslist.jsp").forward(request, response);
        } catch (Exception e) {
